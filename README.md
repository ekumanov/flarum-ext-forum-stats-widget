...
- **Online Guests** — Optional approximate count of unauthenticated visitors browsing the forum, shown as a separate row with a dotted circle below the hidden users. Each guest tab pings a small endpoint that records a hash of the visitor's IP (read from Cloudflare / reverse-proxy headers only when the request genuinely arrives through that proxy, so the value can't be forged) and User-Agent into a short-lived presence map; the displayed count is the number of unique fingerprints seen within the "last seen interval". Counting is approximate by design — visitors behind a shared NAT collapse into one, mobile users on rotating IPs may be over-counted — and **no persistent identifier is stored on the visitor's browser**. Search-engine and other bots are excluded by User-Agent so the count reflects human visitors, not crawlers (notably JS-rendering crawlers, which execute the page and would otherwise trip the ping like a real browser). The endpoint is rate-limited per IP, the in-memory map is hard-capped (oldest evicted on overflow), and the count can optionally be merged into the bar's main "online" number.
